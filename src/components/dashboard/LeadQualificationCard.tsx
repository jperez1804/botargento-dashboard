...
function formatMoney(value: string, currency: string, locale: string): string {
  ...
  const amount = new Intl.NumberFormat(locale, { maximumFractionDigits: 0 }).format(n);
          ...
          {items.map((item) => (
            <div key={item.label} className="flex justify-between gap-3 text-[13px]">
              ...
              <dd className="text-[var(--ink)] text-right min-w-0 break-words">
                {item.format === "money" ? (
                  <span className="tabular-nums">{formatMoney(item.value, item.currency, locale)}</span>
                ) : item.format === "links" ? (
