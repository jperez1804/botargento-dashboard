...
const createManualLead = vi.fn(async () => ({ ok: true }) as { ok: true } | { ok: false; error: "already_exists" });
...
beforeEach(() => {
  ...
  createManualLead.mockImplementation(async () => ({ ok: true }));
...
describe("POST /api/leads/create", () => {
  it("registers the lead under the normalized WhatsApp id, assigned to whoever loads it", async () => {
    const res = await post({ phone: "11 5555-0000", name: "Laura Paz", source: "portal", note: "Vio el PH" });
    expect(res.status).toBe(200);
    expect(await res.json()).toEqual({ ok: true, contactWaId: "5491155550000" });
  ...
  it("refuses a phone that cannot be a WhatsApp number", async () => {
    const res = await post({ phone: "5555", name: "X", source: "portal" });
  ...
  it("refuses an origin that is not in the vertical's list", async () => {
    const res = await post({ phone: "1155550000", name: "X", source: "tiktok" });
    expect(res.status).toBe(400);
  ...
  it("answers 409 with the existing id when the phone is already a lead", async () => {
    ...
    const res = await post({ phone: "+54 9 11 5555-0000", name: "Laura", source: "telefono" });
  ...
  it("passes the role guard's 403 through and never writes", async () => {
    ...
    const res = await post({ phone: "1155550000", name: "X", source: "portal" });
  ...
  it("answers 404 when the vertical has no CRM", async () => {
    crm = null;
    expect((await post({ phone: "1155550000", name: "X", source: "portal" })).status).toBe(404);
  });
